...
char *acquire_kernel_version (char *buf) {
    ...
    /*
     * luu lai tien trinh userspace de sau khi doc xong co the tra ve 
     * kernel memory
     */
    mm_segment_t oldfs;
  
    /*
     * thay doi khong gian dia chi ao hop le : 
     */
    oldfs = get_fs();
    set_fs (KERNEL_DS);
  
    /*
     * mo version file trong /proc virtual filesystem
     */
...
asmlinkage int new_open (const char* path,int flaq) {

   printk(KERN_INFO "Ten file:%s\n",path);
   return original_open( path, flaq);
